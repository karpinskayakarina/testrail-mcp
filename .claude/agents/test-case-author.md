...
4. **For each case build**:
   ...
   - `custom_preconds` — derive content from the requirements + design reports for THIS specific feature. Follow stream-specific preconditions guidance from the rule pack if present (e.g. `streams/funnels-appnebula.md` has a fixed template); otherwise build per-feature based on what the Jira ticket and Figma actually require. Apply global HTML format rules from "HTML Content Validation".
