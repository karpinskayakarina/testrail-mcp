...
> **Codebase awareness — repo-agnostic.** This skill does not require a specific working directory. The funnel implementation files (`src/funnels/...`, `tests/funnels/...`) live in the web app repo (`web-playwright-tests` or whichever app repo holds the funnels). The skill MAY be invoked from there, in which case it can do code lookups; or it MAY be invoked from the testrail-mcp repo, in which case all funnel metadata comes from user Q&A in Step 0b. Detect which mode applies by checking once: `[ -d src/funnels ] && echo has-funnels-code` — store the result as `code_lookup_available`. If false, skip every code-lookup step and ask the user instead.
...
| Outcome | Action |
|---|---|
| Section found | Use its `id` as `section_id`; mark `path = "existing-section"`. Fetch existing cases later for refs/automation_status copy. |
| Section not found | Mark `path = "new-section"`. Confirm display name with the user. **Do NOT create the section yet** — section creation is deferred to Step 7 (after the user approves the generated cases at PAUSE) so that a cancel does not leave an empty section behind. |

> **Why deferred creation:** if the user runs `--draft`, `--dry-run`, or types `cancel` at PAUSE, no section is committed to TestRail. Auto-Jira creation in Step 0c is also deferred to Step 7 for the same reason.

## STEP 0b — funnel metadata Q&A

Walk through the questions below **one at a time**. If `code_lookup_available == true` (Step 0), run the listed lookup first and present the found value; ask the user only to confirm or override. If `code_lookup_available == false`, skip every lookup and ask directly. **Skip questions whose `applies` condition is false** — for a no-scan funnel, items 2 and 7 are skipped, so the actual question count is 9 instead of 11.

| # | Question | Applies | Code lookup (when available) | Notes |
|---|----------|---------|-------------------------------|-------|
| 1 | Photo scan? (yes/no) | always | — | Branching for Case 6 |
| 2 | Scan type? (hand / face) | scan = yes | — | `hand` → title token `palmistry`; `face` → token `face` |
| 3 | Email marketing? (yes/no) | always | — | Cases 9 and 10 depend on this |
| 4 | Subscription prices — successful payment | always | `grep -E "funnelSubscriptions\.\|defaultTrial" src/funnels/test-data/subscription.ts` then `grep -i "{slug}" src/funnels/test-data/subscription.ts` | Funnel-specific function or `defaultTrial1/5/9/13_67`. Present found values; confirm. |
| 5 | Subscription price — payment error | always | same file | Present found values; confirm. |
| 6 | Subscription price — additional discount | always | same file | Present found values; confirm. |
| 7 | Failed scan test price | scan = yes | — | Default 13.67 — propose, confirm. |
| 8 | Email subject | always | `grep -A2 "{slug}" src/funnels/constants/email.ts` | If found → use directly. Else suggest by pattern: scan funnel → `🔮 Get your {Display_Name} and Palmistry Readings`; no scan → `🔮 Get your {Display_Name} Reading`. Confirm. |
| 9 | Email button text (`ReadingEmailButton`) | always | same file | If found → use. Else ask. |
| 10 | Upsell? (no / ULTRA_PACK / CONSULTATION) | always | — | Cases 5 and 7 depend on this |
| 11 | userData fields (gender, DOB, palmReadingGoal, splits) | always (palm goal only if scan = hand) | `ls tests/funnels/{slug}/ 2>/dev/null && grep -r "userData" tests/funnels/{slug}/` | If test files exist with userData defaults — propose, confirm. Else ask one field at a time. |

**Validate prices** against the Standard Subscription Price Mapping in `.claude/skills/_shared/streams/funnels-appnebula.md`:
| Trial | Recurring |
|---|---|
| 1 | 42.99 |
| 5 | 42.99 |
...
## STEP 0c — Jira lookup (no creation yet)

> Only relevant when `path == "new-section"`. Skip entirely on the existing-section path.

Search Jira to detect a pre-existing automation task for this funnel:
```
...
| Result | Action |
|---|---|
| Existing task found | Capture key + URL → store as `jira_existing_task`. Cases will use this key as `refs`. No creation needed. |
| No task found | Mark `jira_create_pending = true`. Ask the user once now: "Should the Jira task be assigned to someone? (name or skip)". If name given, resolve via `lookupJiraAccountId` and store as `jira_assignee_account_id`. **Do NOT call `createJiraIssue` yet** — defer to Step 7 so a cancel does not leave an orphan task. |
| API error | Retry once. If still failing, mark `jira_create_pending = true` with `jira_lookup_failed = true` so Step 7 can re-attempt; report the failure if it persists. |

The actual `createJiraIssue` call and the section description update happen in Step 7 (after PAUSE approval), using the values gathered here.

## STEP 0d — collect update targets (only if `--update`)

> Skip entirely on the new-section path — there are no existing cases yet.

Two paths:
1. User passed explicit case IDs → fetch each via `get_case`.
2. No IDs given → call `get_cases_lite(project_id: 6, suite_id: 486, section_id: <funnel section>)` to fetch all cases in the section. If the section is empty (newly added cases or stale state), warn the user that `--update` will behave like a fresh add for this funnel.

Store as `existing_cases`. For each fixed-title case in Step 4, find the existing case by exact title match; preserve its `refs` and `custom_automation_status` (the rule pack guidance is in the `DO` section of `.claude/skills/_shared/streams/funnels-appnebula.md` — look for the "When creating new cases for a funnel that already has an existing section" bullet). Cases with no old counterpart use the default `custom_automation_status: 3` and empty `refs`.
...
For each case 1–12 (in order from the "Standard Full Set" table in `.claude/skills/_shared/streams/funnels-appnebula.md`):
...
### Preconditions HTML
Build from the template in `.claude/skills/_shared/streams/funnels-appnebula.md` "Preconditions Format" section, substituting:
...
- USA-locale cases: add behavioral notes per `.claude/skills/_shared/streams/funnels-appnebula.md` Case 2 reference (no cookie banner, agreement consent on /gender, no /emailConfirm, manual tests use Safari)
...
- Cases 3–7, 9–11: copy the per-case steps template from `.claude/skills/_shared/streams/funnels-appnebula.md` "Standard Steps per Case Type" section. Wrap each step in `<p>`. Use `<ol>` for multi-action steps, `<ul>` for enumerations.
...
### Custom fields (apply uniformly per `.claude/skills/_shared/streams/funnels-appnebula.md` "Case Fields"):
```
template_id: 2
type_id: 6
priority_id: 4
...
Send the JSON array to the reviewer for an independent QA pass. The reviewer needs the `funnel_metadata` block to validate set-completeness correctly — without `scan` it cannot tell whether 11 cases (no Case 6) is right or whether Case 6 is mistakenly missing.
...
- HTML format
- Set completeness against `funnel_metadata.scan` (12 with scan, 11 without — Case 6 omitted)
- Field defaults
...
## STEP 7 — commit changes to TestRail and Jira

Order matters — section comes first because Jira description needs `section_id`, cases come last because they need `refs = JIRA_KEY`.

### 7a — create section (only on new-section path)

```
section_id = mcp__claude_ai_Testrail_MCP_2__add_section(
  project_id: 6,
  suite_id: 486,
  parent_id: 8648,
  name: "{Display_Name} (AI generated)"
).id
```

If creation fails → abort with the API error. Nothing else has been committed yet, so there's nothing to roll back.

### 7b — auto-Jira creation (only when `jira_create_pending == true`)

```
jira_key = mcp__claude_ai_Atlassian__createJiraIssue(
  cloudId: "676994ec-3063-4a4c-87a0-a41e1b04d5c6",
  projectKey: "AUTOMATION",
  issueTypeName: "Task",
  summary: "Automation / {Display_Name} funnel",
  parent: { key: "AUTOMATION-2953" },
  components: ["Automation"],
  labels: ["automation", "funnels"],
  assignee_account_id: jira_assignee_account_id,
  description: "Automate all test cases in the TestRail section below that have the status \"To Be Automated\".\n\nSection ID: {section_id}\nSection link: https://obrio.testrail.io/index.php?/suites/view/486&group_id={section_id}"
).key
```

On success → set `refs = jira_key` on every case JSON before the upload loop. On failure → retry once. Final fallback: leave `refs` empty on the cases and report the failure in Step 8.

### 7c — update section description with Jira link (new-section path with jira_key)

```
mcp__claude_ai_Testrail_MCP_2__update_section(
  section_id: section_id,
  description: "Jira: https://obrio.atlassian.net/browse/{jira_key}"
)
```
...
- `"new"` → `mcp__claude_ai_Testrail_MCP_2__add_case(section_id, payload)`
- `"updated"` → `mcp__claude_ai_Testrail_MCP_2__update_case(case_id: _existing_id, payload)`
- `"unchanged"` → skip
