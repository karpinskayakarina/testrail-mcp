...
module.exports = function registerSections(server, client) {
  server.tool('get_section', 'Get a single section by ID', {
    section_id: coerceInt.positive().describe('Section ID'),
  }, async ({ section_id }) => {
  ...
  server.tool('get_sections',
    ...
    suite_id: coerceInt.positive().optional().describe('Suite ID. Funnels = 486, Mobile iOS = 136, Mobile Android = 137, AskNebula = 170, API = 1660'),
  ...
  server.tool('add_section',
    ...
    suite_id: coerceInt.positive().optional().describe('Suite ID'),
    parent_id: coerceInt.positive().optional().describe('Parent section ID. AppNebula Funnels = 8648, Quiz funnels = 8694'),
  ...
  server.tool('delete_section', 'Delete a section and ALL its test cases (irreversible). Double-check section_id before calling.', {
    section_id: coerceInt.positive().describe('Section ID to delete'),
  }, async ({ section_id }) => {
